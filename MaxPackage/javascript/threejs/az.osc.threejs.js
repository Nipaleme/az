...
const getCircle = (dist) =>
  new Array(8).fill(0).map((_, index, array) => {
    const angle = (index * 2 * Math.PI) / array.length;
    return new THREE.Vector3(Math.cos(angle) * dist, Math.sin(angle) * dist, 0);
  });
...
const translate = (arrayPos, center) =>
  arrayPos.map((pos) => new THREE.Vector3().addVectors(pos, center));
const rotate = (arrayPos, center) => {
  ...
  return array.map((pos) =>
    new THREE.Vector3(pos.x, pos.y, pos.z).applyEuler(euler)
  );
};
const handlers = {
  ...
  [Max.MESSAGE_TYPES.ALL]: (handled, ...message) => {
    ...
    const { gains, delays } = moved.reduce(
      (prev, curr) => {
        const distEarly = curr.distanceTo(new THREE.Vector3(0, 0, 0));
        const usedDist =
          Math.abs(distCenter - distEarly) + Math.max(distCenter, distEarly);
    ...
    const earlyWidth = [
      `${oscPrefix}/early/width`,
      Math.min(((maxAngle * 180) / Math.PI) * 2, 180),
    ];
    ...
    const clusterDelays = clusterCircle.reduce(
      (prev, curr) => {
